...
import type { Session, ID, User } from "../storage/types";
...
export default function SessionScreen() {
  ...
  function toggleUser(u: User) {
    const s = { ...session, users: session.users.map(x => x.id === u.id ? {...x, active: !x.active} : x) };
    repo.upsertSession(s); setSession(s);
  }
  ...
  function addBowl() {
    ...
    const s = { ...session, bowls: [bowl, ...session.bowls] };
    repo.upsertSession(s); setSession(s);
  }
  ...
  return (
    <div className="wrap">
      <header>
        ...
        <div className="sub">{session.place ?? "—"} • Цена за чашу: {fmtMoney(price)}</div>
      </header>
      ...
      <section>
        ...
        <div className="chips">
          ...
          <button className="chip" onClick={() => {
            ...
            const s = { ...session, users: [...session.users, u] };
            repo.upsertSession(s); setSession(s);
          }}>+ участник</button>
        </div>
      </section>
      ...
      <section>
        <button className="primary" onClick={addBowl}>+ Чаша</button>
        <div className="hint">По умолчанию делится между отмеченными участниками</div>
      </section>
        ...
        <div className="list">
          {session.bowls.map(b => (
            <div className="card" key={b.id}>
              <div>
                <div className="card-title">Чаша • {new Date(b.at).toLocaleTimeString()}</div>
                <div className="card-sub">Участников: {b.participantIds.length}</div>
              </div>
              <div className="sum">{fmtMoney(b.priceRub)}</div>
            </div>
          ))}
          {session.bowls.length === 0 && <div className="empty">Пока нет чаш</div>}
        </div>
      </section>
    </div>
  );
...
function CreateSession({ onCreate }: { onCreate: (s: Session) => void }) {
  ...
  return (
    <div className="wrap">
      <header><div className="title">Новая сессия</div></header>
